from psg_creator.preview_png import write_default_preview, preview_img_path
from psg_creator.gui import *
from psg_creator.classes import *
from psg_creator.element_data import *
from psg_creator.gui import *
from psg_creator.usercode import *
...
class ui_creator():
	...
	def cap(self, win_id=None, title='temp_preview'):
		if win_id is None:
			self.preview_win = self._preview(run=False, title=title)
			self.preview_win.read(timeout=1)
			win_id = self.get_preview_win_id(title=title)
	...
	def get_preview_win_id(self, title='Preview'):
		info = self.win_info()
		for line in info:
	...
	def get_preview_img(self, map_data=None):
		if map_data is None:
			print(f"Set preview_data to self.map: {self.map}")
			self.preview_data = self.map.map
		else:
			print(f"set preview data from arg: {map_data}")
			self.preview_data = map_data
	...
	def load_ui(self, filepath=None, run=False):
		...
		if run:
			self._preview(data=self.preview_data, run=run)
		...
		while True:
			...
			if self.key == sg.WINDOW_CLOSED:
				...
				try:
					...
					log(f"creator._win_add_element(): Error - dtype conversion failed! ({values[self.key]})", 'error')
					try:
						self.value = values[key]
					except Exception as e:
						log(f"creator._win_add_element(): Error - couldn't set vaue! ({e})", 'error')
						self.value = None
				if self.key == '-WIDTH-':
					self.key = '-SIZE-'
	...
	def _preview(self, data=None, title='Preview', run=True, finalize=True):
		try:
			wc = self.main_window.was_closed()
		except:
			wc = False
		...
		if not wc:
			try:
				#print(self.main_window, dir(self.main_window))
				location = self.main_window.current_location()
			except Exception as e:
				print("Error getting current location:", e)
				location = (None, None)
		else:
			location = (None, None)
		self.preview_win = self.ui.child_window(layout_obj=layout_obj, title=title, run=self.run_preview, location=location, finalize=finalize)
		e, v = self.preview_win.read(timeout=1)
		self.preview_win.metadata = {'targets': self.functions} # store function objects to window metadata
		try:
			wc = self.preview_win.was_closed()
		except:
			wc = False
		if not run:
			...
			if e == sg.WINDOW_CLOSED:
				...
				if e in self.functions.keys(): # if element has a registered function object...
					...
					if target_key is not None:
						tdata = self.dtype._to_type(target_key) # grab source and destination element keys (convert type from string)
						#print("tdata:", tdata)
					else:
						tdata = None
						src = None
						target = None
					if type(tdata) == tuple or type(tdata) == list: # if more than one key provided, set src and target
						src, target = tdata
	...
	def update_preview_img(self, data=None):
		if data is None:
			data = self.map.map
		print("updating preview image")
		img = self.get_preview_img(map_data=data)
		if not self.main_window.was_closed():
			self.main_window['-PREVIEW_IMG-'].update(source=img)

	def create_elements_dict(self):
		defaults = self.defaults
		for element_name, element_data in self.preview_data:
			#print(element_data)
			if element_name != 'push':
				for key in defaults[element_name].keys():
					if key not in list(element_data.keys()):
						print("key not in element_data:", key)
						element_data[key] = defaults[element_name][key]
				self.elements[element_name] = element_data
		return self.elements


	def main(self):
		exit = False
		self.main_window = self.ui._win_main()
		self.update_preview_img()
		elements = []
		self.selected = None
		while True:
			...
			if event == sg.WINDOW_CLOSED:
				...
				self.preview_data = self.load_ui(filepath=filepath, run=False)
				self.create_elements_dict()
				#self.main_window['-ELEMENTS_LIST-'].update(self.preview_data)
				self.main_window['-ELEMENTS_LIST-'].update(list(self.elements.keys()))
				self.update_preview_img()
